...
struct claim parse_claim(string const line) {
  ...
  if (string_length(line) > BUFFER_LEN - 1) {
    fprintf(stderr, "ill formed string; too long (%zu)", string_length(line));
    return ret;
  }
  ...
  int places_filled = sscanf(buffer, "#%d @ %d,%d: %dx%d", &ret.id, &ret.left,
                             &ret.top, &ret.width, &ret.height);
...
void do_the_thing(string const data, enum part part) {
  ...
  db_for_each(struct claim const, it, claims) {
    range_for_each(int, y, it->top, it->top + it->height) {
      range_for_each(int, x, it->left, it->left + it->width) { ++fabric[y][x]; }
    }
  }
