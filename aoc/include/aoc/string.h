...
#ifdef _MSC_VER
int str_scanf(string s, _Scanf_format_string_params_(0) char const *fmt, ...);
#else
...
#define string_for_each(name, self) slice_for_each(char, name, s.start, s.end)
#define lines_for_each(name, data)                                             \
  for (string name = {0}, __##name##_rest = (data);                            \
       _Aoc_lines_iterator_next(&name, &(__##name##_rest));)
