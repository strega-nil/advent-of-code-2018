...
static struct dynamic_buffer_repr* repr_of(void const* buffer) {
	...
	return (
			struct
			dynamic_buffer_repr*)(buffer_ptr - offsetof(struct dynamic_buffer_repr, buffer));
}
...
void _Aoc_dynamic_buffer_clear(void* self) {
	repr_of(self)->length = 0;
}

void _Aoc_dynamic_buffer_remove(
		void* self, size_t idx, size_t el_size) {
...
void* _Aoc_dynamic_buffer_resize(
		void* self, size_t el_size, size_t capacity) {
